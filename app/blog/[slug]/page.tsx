...
function extractHeadings(markdown: string): TocHeading[] {
  ...
  while ((m = re.exec(markdown)) !== null) {
    ...
    headings.push({ id: slugifyHeading(raw), text, level });
  }
  return headings;
}
...
function splitAtMidpoint(html: string): [string, string] | null {
  const re = /<\/p>/g;
  const positions: number[] = [];
  let m;
  while ((m = re.exec(html)) !== null) positions.push(m.index + 4);
  if (positions.length < 8) return null;
  const mid = positions[Math.floor(positions.length * 0.45)];
...
export default async function BlogPostPage({
  ...
  const rawHtml = marked.parse(contentWithoutH1) as string;
  const html = addHeadingIds(rawHtml);

  const headings = extractHeadings(contentWithoutH1);
  const readMinutes = parseInt(post.readTime) || 0;
  const splitHtml = readMinutes >= 10 ? splitAtMidpoint(html) : null;
    ...
    <>
      ...
      <div className="min-h-screen bg-[oklch(0.1_0.005_265)]">
        ...
        <main id="article-main" className="pt-28 pb-24" tabIndex={-1}>
          {/* Outer: wider on xl to accommodate sidebar */}
          <div className="max-w-[1060px] mx-auto px-4 md:px-6">
            {/* Grid: article + optional ToC sidebar */}
            <div className={headings.length >= 3 ? "xl:grid xl:grid-cols-[minmax(0,1fr)_220px] xl:gap-14 xl:items-start" : ""}>
              ...
              <div className="min-w-0">
                ...
                {/* Article body — optionally split with inline CTA */}
                {splitHtml ? (
                  <>
                    <div className={PROSE} dangerouslySetInnerHTML={{ __html: splitHtml[0] }} />

                    {/* Inline CTA */}
                    <div className="my-12 p-6 rounded-xl border border-indigo-500/25 bg-indigo-600/5 not-prose">
                      <p className="text-xs font-mono text-indigo-400 uppercase tracking-widest mb-2">
                        Working on something like this?
                      </p>
                      <p className="text-white font-semibold mb-3 text-lg" style={{ fontFamily: "var(--font-display)" }}>
                        Let&apos;s talk about your AI product.
                      </p>
                      <p className="text-zinc-400 text-sm mb-4 leading-relaxed">
                        I embed with teams as a fractional AI Product Leader — prototype in 48 hours, own the roadmap, ship with documented outcomes.
                      </p>
                      <a
                        href="/#contact"
                        className="inline-flex items-center gap-2 px-5 py-2.5 bg-indigo-600 hover:bg-indigo-500 text-white font-medium rounded-md transition-colors text-sm"
                      >
                        Book a discovery call <ArrowRight size={14} />
                      </a>
                    </div>

                    <div className={PROSE} dangerouslySetInnerHTML={{ __html: splitHtml[1] }} />
                  </>
                ) : (
                  <div className={PROSE} dangerouslySetInnerHTML={{ __html: html }} />
                )}

                <div className="flex flex-wrap gap-2 mt-12 pt-8 border-t border-white/8" aria-label="Article tags">
                  {post.tags.map((tag) => (
                    <span
                      key={tag}
                      className="flex items-center gap-1 text-xs text-zinc-500 bg-white/5 px-3 py-2 rounded-full"
                    >
                    ...
                    <div className="grid md:grid-cols-2 gap-4">
                      {related.map((r) => (
                        <Link
                          ...
                          className="p-4 rounded-xl border border-white/8 hover:border-indigo-500/30 bg-[oklch(0.14_0.008_265)] hover:bg-[oklch(0.15_0.01_265)] transition-all block"
                        >
